...
class ServiceContainer:
    def __init__(self, settings: Settings) -> None:
        ...
        self.messages = MessageService(self.store, self.notifications)
        self.realtime = RealtimeManager()
        self.auth = AuthService(self.store, self.notifications, self.email)
        self.auctions = AuctionService(self.store, self.notifications, self.realtime, self.messages)
        self.consignments = ConsignmentService(self.store, self.notifications, self.messages)
        self.metrics = MetricsService(self.store, self.notifications)
        self.admin = AdminService(self.store, self.auctions, self.notifications, self.email)
